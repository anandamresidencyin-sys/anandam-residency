// app/api/send-plot-enquiry/route.ts
...
export async function POST(request: Request) {
  try {
    ...
    console.log('Received plot enquiry:', { name, email, phone, plotName, plotId });

    // Validate required fields
    if (!name || !email || !phone || !plotName) {
    ...
    const mailOptions = {
      ...
      subject: `🏠 NEW PLOT ENQUIRY: ${plotName} - ${name}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="UTF-8">
          <title>New Plot Enquiry</title>
          <style>
            ...
            .plot-card { background: linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%); padding: 15px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid #79c223; }
        ...
        <body>
          <div class="header"><h2>🏢 Anandam Residency</h2><p>🎯 New Plot Enquiry Received</p></div>
          <div class="content">
            <div class="plot-card">
              <div class="label">🏠 PLOT DETAILS</div>
              <div class="value"><strong>📌 Plot Name:</strong> ${plotName}</div>
              <div class="value"><strong>🆔 Plot ID:</strong> ${plotId}</div>
              <div class="value"><strong>🔢 Plot Number:</strong> ${plotNumber || 'N/A'}</div>
              <div class="value"><strong>📐 Plot Size:</strong> ${plotSize || 'N/A'}</div>
              <div class="value"><strong>💰 Price:</strong> ${plotPrice || 'N/A'}</div>
              <div class="value"><strong>📍 Location:</strong> ${plotLocation || 'N/A'}</div>
              <div class="value"><strong>🧭 Facing:</strong> ${plotFacing || 'N/A'}</div>
              <div class="value"><strong>🏗️ Project:</strong> ${projectName || 'N/A'}</div>
            </div>
          ...
          <div class="footer"><p>This customer is interested in <strong>${plotName}</strong>. Please respond within 24 hours.</p><p>© ${new Date().getFullYear()} Anandam Residency</p></div>
        </body>
        </html>
      `,
      replyTo: email,
    ...
    const autoReplyOptions = {
      from: `"Anandam Residency" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: `Thank you for your enquiry about ${plotName} - Anandam Residency`,
      html: `
        <!DOCTYPE html>
        <html>
        ...
        <body>
          ...
          <div class="content">
            <h3>Dear ${name},</h3>
            <p>Thank you for your interest in <strong>${plotName}</strong>!</p>
            <div class="info-box">
              <p><strong>✅ Your enquiry has been received successfully.</strong></p>
              <p><strong>🏠 Plot:</strong> ${plotName}</p>
              <p><strong>📍 Location:</strong> ${plotLocation || 'N/A'}</p>
              <p><strong>💰 Price:</strong> ${plotPrice || 'N/A'}</p>
            </div>
            <p><strong>⚡ What happens next?</strong></p>
            <ol><li>Our plot specialist will review your request</li><li>You will receive a call within <strong>24 hours</strong></li><li>Detailed plot brochure will be shared via email</li></ol>
